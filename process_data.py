		...
		for j in range(len(sheets)):
			...
			if (~sheets[j].columns.str.contains('CI N',case=False).any()):
				...
				if np.shape(sites_itsm)[0]>0:
					...
					if np.shape(existing_sites)[0]>0:
						...
						print('New sites to upload in CMDB: '.upper(),'-'*len('New Sites to upload in CMDB: '),'#: ' + str(np.shape(existing_sites)[0]),'',sep='\n',file=open(report +'issues.txt','a',encoding='utf8'))
			...
			if sheets[j].columns.str.contains('CI N',case=False).any():
				...
				if np.shape(cis_chars)[0]>0:
					...
					None
				print('PRIORITIES USED: ','-'*len('PRIORITIES USED: '),pd.DataFrame(np.unique(cis[0].filter(regex=re.compile('PRIORITY',re.IGNORECASE)))),'',sep='\n',file=open(report +'issues.txt','a',encoding='utf8'))
