	...
	if len(list(itertools.chain(*unmatched_fields)))>0:
		...
		for j in range(len(sheets)):
			...
			for i in range(len(sheets[j].columns)):
				...
				count_chars.append(sheets[j].iloc[:,i].astype(str).apply(lambda x: x if pd.isnull(x) else (len(str(round(x,5))) if type(x)==float else len(str(x)))).max())
			...
			if sheets[j].columns.str.contains('CI N',case=False).any():
				...
				same_prod=prodcats_cis[~prodcats_cis['Product Name'].isna()].merge(template,left_on=prod_name.name,right_on=template.columns[3],how='inner').drop_duplicates()
